...
function product_shortcode($args, $content) {
	...
	$des = $args['des'] ;
	ob_start();
	$args = array( 'post_type' => 'expertise', 'posts_per_page' => 12, /*'orderby'=> 'title', */'order' => 'ASC' );
	...
	<div class="slhome_list slhome_list_sliderhome left_0 col-lg-5ths">
		...
		<div class="dot-slider">
			<ol class="">
				...
				while ($query2->have_posts()) {
				$query2->the_post();?>
					<li data-slide-to="<?php the_field('data_slide');?>"
						class="slhome_title_sliderhome slhome_title slhp_<?php the_field('data_slide');?>
						<?php if($i==1){echo 'current';}?>">

						<?php
						$link = '#';
						if($pageId = get_field('key_service_link_page')) {
							$link = '/?page_id=' . $pageId;
						}
						?>
