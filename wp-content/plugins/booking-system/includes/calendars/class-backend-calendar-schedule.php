                ...
                if($settings_calendar->google_enabled == 'true'
                  ...
                  && $firstYear == '\"false\"') {
                    ...
                    foreach($events as $event) {
                        $check_in = strtotime($event['DTSTART']);
                        
                        if($event['DTSTART'] != $event['DTEND']) {
                            $check_out = strtotime($event['DTEND'])-86400;
                        }
                        $start_time = date('H',$check_in).':'.date('i',$check_in);
                        $end_time = date('H',$check_out).':'.date('i',$check_out);
                        $days = array();
                        
                        // Same Day
                        if($check_in == $check_out
                          || $check_out <= $check_in+60*60*24) {
                            $day = date('Y',$check_in).'-'.(date('n',$check_in) < 10 ? '0'.date('n',$check_in):date('n',$check_in)).'-'.(date('j',$check_in) < 10 ? '0'.date('j',$check_in):date('j',$check_in));
                            
                            if (array_key_exists($day, $schedule)) {
                                $day_decode = json_decode($schedule[$day]);

                                if($settings_calendar->hours_enabled == 'true') {
                                    
                                    foreach($day_decode->hours as $key => $value) {
                                        
                                        if($key >= $start_time
                                          && $key < $end_time) {
                                            $day_decode->hours->{$key}->available = $day_decode->hours->{$key}->available-1;
                                            
                                            if($day_decode->hours->{$key}->available < 1) {
                                                $day_decode->hours->{$key}->status = 'booked';
                                                $day_decode->hours->{$key}->price = 0;
                                            }
                                            $day_decode->available = $day_decode->available-1;

                                            if($day_decode->available < 1) {
                                                $day_decode->status = 'booked';
                                                $day_decode->price = 0;
                                            }
                                        }
                                    }
                                ...
                                $schedule[$day] = json_encode($day_decode);
                            } else {
                                $schedule[$day] = '{"available":1,"bind":0,"hours":{"00:00":{"available":0,"bind":0,"info":"","notes":"","price":0,"promo":0,"status":"none"}},"hours_definitions":[{"value":"00:00"}],"info":"","notes":"","price":0,"promo":0,"status":"booked"}';
                            }
                        }
                        
                        for($i = $check_in; $i<= $check_out; $i = $i + 60*60*24){
                            $day = date('Y',$i).'-'.(date('n',$i) < 10 ? '0'.date('n',$i):date('n',$i)).'-'.(date('j',$i) < 10 ? '0'.date('j',$i):date('j',$i));
